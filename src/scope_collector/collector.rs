//! The forward-pass AST walker: the `Collector` state and the `walk_*`
//! functions that record every variable read and write with byte
//! offsets.  Driven by the constructors in [`super::build`].
...
pub(super) struct Collector<'a> {
    ...
    pub(super) enclosing_class_name: Option<String>,
}
...
pub(super) fn walk_statement(stmt: &Statement<'_>, collector: &mut Collector<'_>) {
    match stmt {
...
pub(super) fn walk_expression(expr: &Expression<'_>, collector: &mut Collector<'_>) {
    match expr {
