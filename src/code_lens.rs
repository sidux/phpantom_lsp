...
//!
//! Shows compact, clickable annotations for references, inheritance, and
...
use crate::framework::{FrameworkClassReferenceSource, FrameworkReferenceKind, SymfonySymbolKind};
use crate::reference_index::ReferenceIndexKey;
use crate::references::members::{
...
impl Backend {
    ...
    ///
    /// Returns inheritance and framework relationship lenses for indexed PHP
    /// symbols.
    pub fn handle_code_lens(&self, uri: &str, content: &str) -> Option<Vec<CodeLens>> {
        ...
        for class in &classes {
            ...
            let implementation_descendants = if self.workspace_indexed.load(Ordering::Acquire) {
                self.implementation_descendants(class, &class_loader, true)
            } else {
                Vec::new()
            };

            if let Some(source_pos) = class_lens_position(content, class) {
                self.push_declaration_reference_lens(
                    uri,
                    source_pos,
                    Some(&class_fqn),
                    &mut lenses,
            ...
            if !implementation_descendants.is_empty()
                && let Some(source_pos) = class_lens_position(content, class)
            {
            ...
            for method in &class.methods {
                ...
                if !method.name.starts_with("__") && proto.is_none() {
                    self.push_member_reference_lens(
                        uri,
                        pos,
                        method.name_offset,
                        class_fqn,
            ...
            for property in &class.properties {
                ...
                let pos = offset_to_position(content, property.name_offset as usize);
                if !property.is_virtual
                    && property.visibility != crate::types::Visibility::Private
                    && !self.property_has_prototype(class, &property.name)
                {
                    self.push_member_reference_lens(
                        uri,
                        pos,
            ...
            for constant in &class.constants {
                if constant.name_offset == 0
                    || constant.is_virtual
                    || constant.visibility == crate::types::Visibility::Private
                    || self.constant_has_prototype(class, &constant.name)
                {
                    continue;
                }
                self.push_member_reference_lens(
                    uri,
                    offset_to_position(content, constant.name_offset as usize),
    ...
    fn push_framework_class_lenses(
        ...
        let mut symfony_service_locations = Vec::new();
        let mut symfony_config_locations = Vec::new();
        let mut doctrine_locations = Vec::new();
        let mut openapi_locations: BTreeMap<String, Vec<Location>> = BTreeMap::new();
        for reference in self.framework_class_reference_locations_by_source(&class_fqn) {
            match reference.source {
                FrameworkClassReferenceSource::SymfonyService => {
                    symfony_service_locations.push(reference.location);
                }
                FrameworkClassReferenceSource::SymfonyConfig => {
                    symfony_config_locations.push(reference.location);
                }
                FrameworkClassReferenceSource::Doctrine => {
                    doctrine_locations.push(reference.location);
                }
        ...
        for (label, locations) in [
            ("Symfony service", symfony_service_locations),
            ("Symfony config", symfony_config_locations),
            ("Doctrine mapping", doctrine_locations),
        ] {
            if locations.is_empty() {
                continue;
            }
            self.push_locations_lens(
                uri,
                source_pos,
                reference_lens_title(label, locations.len()),
                locations,
                &mut *lenses,
                &mut *seen,
            );
        }
        for (mut title, locations) in openapi_locations {
            if locations.len() > 1 {
                title.push_str(&format!(" · {} refs", locations.len()));
            }
            self.push_locations_lens(uri, source_pos, title, locations, &mut *lenses, &mut *seen);
        }
    ...
    fn push_declaration_reference_lens(
        &self,
        origin_uri: &str,
        source_pos: Position,
        ...
        let range = Range::new(
            Position::new(source_pos.line, 0),
            Position::new(source_pos.line, 0),
        );
        let kind = if class_fqn.is_some() {
            "classReferences"
        } else {
        ...
        let key = format!("{}:{}:{kind}", source_pos.line, source_pos.character);
        if seen.insert(key) {
            lenses.push(lens);
        }
    }
    ...
    fn push_member_reference_lens(
        ...
        let Some(candidate_count) = self.indexed_reference_count(&key) else {
            return;
        };
        let range = Range::new(
            Position::new(source_pos.line, 0),
        ...
        let lens_key = format!(
            "{}:{}:memberReferences",
            source_pos.line, source_pos.character
        );
        if !seen.insert(lens_key) {
            return;
        }
        let Ok(origin_url) = Url::parse(origin_uri) else {
            return;
        ...
        if candidate_count == 0 {
            lenses.push(CodeLens {
                range,
                command: Some(self.build_code_lens_locations_command(
                    reference_count_title(0),
                    origin_url,
                    source_pos,
                    Vec::new(),
                )),
        ...
        let supports_refresh = self.supports_code_lens_refresh.load(Ordering::Acquire);
        let cached = if supports_refresh {
        ...
        if let Some(locations) = cached {
            lenses.push(CodeLens {
                range,
                command: Some(self.build_code_lens_locations_command(
                    reference_count_title(locations.len()),
                    origin_url,
                    source_pos,
                    locations,
                )),
        ...
        // Refresh-capable clients re-pull once the shared batch worker has
        // warmed the exact cache. Omitting the cold lens prevents an eager
        // resolve request for every declaration in the viewport.
        if supports_refresh {
            return;
        }
        lenses.push(CodeLens {
    ...
    pub(crate) fn resolve_code_lens_item(&self, mut lens: CodeLens) -> CodeLens {
        ...
        // Exact reference resolution can touch many files. Code lenses use the
        // LSP's lazy resolve path so opening a document remains fast and only
        // visible declaration lenses pay this cost.
        let locations = match kind {
            "classReferences" => data
                .get("classFqn")
                .and_then(serde_json::Value::as_str)
                .map(|fqn| self.find_class_references(fqn, false)),
            "declarationReferences" => {
        ...
        let Ok(origin_url) = Url::parse(uri) else {
            return lens;
        };
        lens.command = Some(self.build_code_lens_locations_command(
            reference_count_title(locations.len()),
            origin_url,
            position,
            locations,
    ...
    fn property_has_prototype(&self, class: &ClassInfo, property_name: &str) -> bool {
        if self.traits_have_property(&class.used_traits, property_name, 0) {
            return true;
        }
        ...
        for _ in 0..MAX_INHERITANCE_DEPTH {
            ...
            if parent
                ...
                || self.traits_have_property(&parent.used_traits, property_name, 0)
            {
                return true;
            }
            current = ClassInfo::clone(&parent);
    ...
    fn constant_has_prototype(&self, class: &ClassInfo, constant_name: &str) -> bool {
        if self.traits_have_constant(&class.used_traits, constant_name, 0) {
            return true;
        }
        ...
        for _ in 0..MAX_INHERITANCE_DEPTH {
            ...
            if parent
                .constants
                .iter()
                .any(|constant| constant.name == constant_name)
    ...
    pub(crate) fn build_code_lens_locations_command(
        &self,
        title: String,
        origin_uri: Url,
...
fn reference_lens_title(label: &str, count: usize) -> String {
    format!(
        "{label}: {count} {}",
...
fn openapi_lens_title(
    method: Option<&str>,
    path: Option<&str>,
    operation_id: Option<&str>,
) -> String {
    let mut title = String::from("OpenAPI");
    if let Some(method) = method {
        title.push(' ');
        title.push_str(method);
    }
    ...
    if let Some(operation_id) = operation_id {
        title.push_str(" · ");
        title.push_str(operation_id);
    }
    if method.is_none() && path.is_none() && operation_id.is_none() {
        title.push_str(" link");
    }
    title
}
