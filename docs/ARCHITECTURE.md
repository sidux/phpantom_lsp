...
A few tags name a symbol without naming a type. `@see`, and PHPUnit's `@covers` / `@uses` / `@coversDefaultClass`, hold a free-text reference that `symbol_map/docblock.rs` splits into its class and member parts. Coverage metadata adds one shape the others do not have: `@covers ::name` is a global function on its own, and a method of the class a `@coversDefaultClass` names when there is one in scope. That default is read off the class docblock before its tags are walked (so tag order does not matter) and carried down the `ExtractionCtx` to the docblocks of the class's methods. The attribute spelling of the same metadata is handled in `symbol_map/extraction/phpunit.rs`: `#[CoversClass(Foo::class)]` needs nothing special because `Foo::class` is an ordinary expression, but the targets PHPUnit spells as strings (`#[CoversMethod(Foo::class, 'bar')]`, `#[CoversFunction('baz')]`) get their spans there.
