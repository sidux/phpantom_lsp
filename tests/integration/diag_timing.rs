...
/// branch exits, and resolving it sends the property pipeline back over
/// the whole body looking for a check on that call.  The re-walk meets
/// the *other* conditionals, resolves their chains, and each of those
/// starts a walk of its own under a different key, so the walks nest one
/// per distinct chain.  Blocking a walk while another is already running
/// over the same source keeps that flat: before it, 6 chains measured
/// 1.5 s and 8 never finished, which is what made
/// `phpstan-src`'s `AnalyseCommand::execute` unanalysable.
#[test]
fn chained_calls_in_conditionals_do_not_nest_narrowing_walks() {
    const CHAINS: usize = 20;

    let mut php = String::from(
        r#"<?php
class Style  { public function error(string $m): void {} }
class Out    { public function getStyle(): Style { return new Style(); } }
class Report {
"#,
    );
    for i in 0..CHAINS {
        php.push_str(&format!(
            "    public function get{i}(): Out {{ return new Out(); }}\n"
        ));
    }
    php.push_str("}\n\nfunction probe(Report $r, bool $flag): void {\n");
    for i in 0..CHAINS {
        php.push_str(&format!(
            "    if ($flag) {{ $r->get{i}()->getStyle()->error(\"m{i}\"); }}\n"
        ));
    }
    php.push_str("}\n");

    let uri = "file:///test/nested_narrowing_walks.php";
    let backend = create_test_backend();
    backend.update_ast(uri, &php);
    ...
    eprintln!();
    eprintln!("=== Chained calls in conditionals ===");
    eprintln!(
        "  {} chains: {:>10.3?}  ({} diagnostics)",
        CHAINS,
        elapsed,
        out.len()
    );
    ...
    // Budget: 10 s in debug, 2 s in release.  With the walks kept flat
    // this measures well under a second either way, while the nesting
    // never finished at 8 chains, so both budgets catch a regression
    // with room to spare on a slower machine.
    let budget_secs = if cfg!(debug_assertions) { 10.0 } else { 2.0 };
    assert!(
        elapsed.as_secs_f64() < budget_secs,
        "Diagnosing {CHAINS} chained calls took {elapsed:.3?} which exceeds \
         the {budget_secs:.0} s budget.  Narrowing walks may be nesting \
